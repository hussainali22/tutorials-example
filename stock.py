...
class StockMove(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('state') == 'assigned':
            ...
            order_ids = mro_obj.search(cr, uid, [('procurement_group_id', 'in', [x.origin for x in moves])])
            # order_ids = mro_obj.search(cr, uid, [('parts_lines.parts_id', 'in', [x.product_id.id for x in moves])])
